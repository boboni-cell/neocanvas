...
- 如果下拉中没有模型，需要先打开配置弹窗拉取模型列表，并设置默认生图模型和默认文本模型。火山方舟 Ark 若未提供 `/models` 接口，请手动填写模型名或 Endpoint ID。
...
- 使用火山方舟 Ark / Seedance 2.0 时，Base URL 配置为 `https://ark.cn-beijing.volces.com/api/v3`（按量计费），模型名使用 Seedance 2.0 对应模型；系统会改用 `POST /contents/generations/tasks` 创建异步任务，并轮询 `GET /contents/generations/tasks/{id}`。
- 火山方舟 Ark 当前未提供 OpenAI `/models` 模型列表接口，后台不会伪造模型列表；请手动填写 `doubao-seedance-2.0` 或文档列出的其他可用模型。
