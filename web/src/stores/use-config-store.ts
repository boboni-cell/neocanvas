...
function inferChannelCapability(models: string[] | undefined): ModelCapability {
    const list = Array.isArray(models) ? models : [];
    if (list.some((model) => modelMatchesCapability(model, "image"))) return "image";
    if (list.some((model) => modelMatchesCapability(model, "video"))) return "video";
    if (list.some((model) => modelMatchesCapability(model, "audio"))) return "audio";
    return "text";
}
